<template>
  <div class="container">
    ...
    <div class="main-content">
      <AppHeader @refresh="fetchInbox" @compose="navigateTo('compose')" />
      ...
      <div class="content-area">
        <div class="section">
          ...
          <div class="mail-list">
            ...
            <div v-if="!loading && mails.length === 0" class="empty">收件箱为空</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const fetchInbox = async () => {
      try {
        loading.value = true
        mails.value = await emailApi.getInbox()
      } catch (error) {
    ...
    const viewMail = async (mail) => {
      try {
        ...
        if (!mail.read) {
          ...
          // 更新本地邮件状态
          const mailIndex = mails.value.findIndex(m => m.id === mail.id)
          if (mailIndex !== -1) {
    ...
    const toggleStar = async (mail) => {
      try {
        ...
        const mailIndex = mails.value.findIndex(m => m.id === mail.id)
        if (mailIndex !== -1) {
          mails.value[mailIndex].starred = !mails.value[mailIndex].starred
        }
      } catch (error) {
