      ...
      <div class="content-area">
        <div class="section">
          ...
          <div class="mail-list">
            ...
            <div v-if="!loading && searchResults.length === 0" class="empty">没有找到匹配的邮件</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const searchMails = async (searchTerm) => {
      try {
        loading.value = true
        searchResults.value = await emailApi.searchMail(searchTerm)
      } catch (error) {
    ...
    const toggleStar = async (mail) => {
      try {
        ...
        // 更新本地邮件状态
        const mailIndex = searchResults.value.findIndex(m => m.id === mail.id)
        if (mailIndex !== -1) {
          searchResults.value[mailIndex].starred = !searchResults.value[mailIndex].starred
        }
      } catch (error) {
